...
describe('CheckInService Unit Test for Visitor User', () => {
  ...
  it('❌ Should NOT VISITOR user to check-in in private parking', async () => {
    ...
    const validatorMock = {
      canUserEnter: jest.fn().mockReturnValue({
        canEnter: false,
        message: 'Only corporate users can enter private parkings'
      })
    };
    ...
    mockCheckInRepository.save.mockResolvedValue({
      ...
      userType: baseDto.userType,
      accessGranted: false,
      reason: 'Only corporate users can enter private parkings'
    });
  
    const result = await checkInService.handleCheckIn(dto);
  
    expect(result.accessGranted).toBe(false);
    expect(result.reason).toBe('Only corporate users can enter private parkings');
    expect(mockCheckInRepository.save).toHaveBeenCalled();
  });
  ...
  it('❌ Should NOT allow VISITOR user to check-in in courtesy parking on weekdays', async () => {
    ...
    const validatorMock = {
      canUserEnter: jest.fn().mockReturnValue({
        canEnter: false,
        message: 'Courtesy parkings are only available on weekends'
      })
    };
    ...
    mockCheckInRepository.save.mockResolvedValue({
      ...
      userType: baseDto.userType,
      accessGranted: false,
      reason: 'Courtesy parkings are only available on weekends'
    });
  
    const result = await checkInService.handleCheckIn(dto);
  
    expect(result.accessGranted).toBe(false);
    expect(result.reason).toBe('Courtesy parkings are only available on weekends');
    expect(mockCheckInRepository.save).toHaveBeenCalled();
  });  

  it('✅ Should allow VISITOR user to check-in in courtesy parking on weekends', async () => {
    const parking = { id: 'private-id', parkingType: ParkingType.COURTESY };
    const dto: CreateCheckInDto = { ...baseDto, parkingId: parking.id };
